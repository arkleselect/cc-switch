...
#[test]
fn provider_service_delete_current_provider_clears_current_and_succeeds() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let _home = ensure_test_home();
    ...
    ProviderService::delete(&app_state, AppType::Claude, "keep")
        .expect("deleting current provider should succeed");

    let current = app_state
        .db
        .get_current_provider(AppType::Claude.as_str())
        .expect("get current provider after delete");
    assert!(current.is_none(), "current provider should be cleared");
